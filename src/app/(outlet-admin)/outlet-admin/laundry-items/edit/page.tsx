...
export default function OutletAdminEditLaundryItemPage() {
  const router = useRouter();
  const { selectedLaundryItemId } = useLaundryItemStore();
  
  const id = selectedLaundryItemId as string;
  const { formik, isLoading, isFetching } = useUpdateLaundryItem(id, "/outlet-admin/laundry-items");

  useEffect(() => {
    if (!id) {
      router.push("/outlet-admin/laundry-items");
    }
  }, [id, router]);
