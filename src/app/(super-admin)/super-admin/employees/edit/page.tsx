...
export default function EditEmployeePage() {
  ...
  const editingEmployeeId = useEmployeeStore((state) => state.editingEmployeeId);
  
  useEffect(() => {
    if (!editingEmployeeId) {
      router.replace("/super-admin/employees");
    }
  }, [editingEmployeeId, router]);

  const { formik, isLoading, fetching } = useUpdateEmployee(editingEmployeeId as string);
  ...
  if (fetching || !editingEmployeeId) return <div className="p-8 text-center text-gray-500">Loading details...</div>;
