...
export function WorkerStreamStatus() {
  const { notifications } = useWorkerStore();
  ...
  return (
    <div className="flex items-center gap-2">
      {/* Status Indicator (Simple for now as real SSE is not yet implemented) */}
      <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-[#E8E2DA]">
        <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
        <span className="text-xs font-medium text-[#6B6662]">Live</span>
      </div>
